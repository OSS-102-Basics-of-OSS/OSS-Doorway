...
import fs from "fs";
import { handleMCQ, handleCustom } from "./handlers/genericHandlers.js";
...
// Insert Q4T1 handler and Q4 mapping as in XYZ-OSS-Doorway
...
const specificHandlers = {
    ...
    handleQ2T1, handleQ2T2, handleQ2T3, handleQ2T4, handleQ2T5: handleQ2Quiz, handleQ2Quiz,
    handleQ3T1, handleQ3T2, handleQ3T3, handleQ3T4: handleQ3Quiz, handleQ3Quiz, handleQ4T1,
    handleQ5T1
};
  ...
  for (const [taskId, taskData] of Object.entries(questData)) {
    ...
    switch (taskData.type) {
      case "mcq":
        ...
        taskMapping[questId][taskId] = (user_data, user, context, ossRepo, response, selectedIssue, db) => 
      ...
      case "general":
        // Fallback to existing specific handlers
        const handlerName = `handle${questId}${taskId}`;
        if (specificHandlers[handlerName]) {
          taskMapping[questId][taskId] = specificHandlers[handlerName];
        } else {
          console.error(`Handler ${handlerName} not found for ${questId}${taskId}`);
          // Fallback to a default handler
          taskMapping[questId][taskId] = (user_data, user, context, ossRepo, response, selectedIssue, db) => {
            console.error(`No handler available for ${questId}${taskId}`);
      ...
      default:
        console.error(`Unknown task type: ${taskData.type} for ${questId}${taskId}`);
        taskMapping[questId][taskId] = (user_data, user, context, ossRepo, response, selectedIssue, db) => {
          console.error(`No handler available for ${questId}${taskId}`);
          return [response.error, false];
        };
